...
(async()=>{
  ...
  try{
    ...
    await page.locator('#tables-panel [data-mode="rebuild"]').click();
    ...
    assert.equal(await page.locator('#tables-panel [data-mode="review"]').innerText(),'Review · 1');
    await page.reload();await page.locator('#tables-panel [data-mode="review"]').click();
    ...
    await page.locator('[data-cell="definite:0:0"]').fill('der');await page.keyboard.press('Enter');
    assert.equal(await page.locator('#tables-panel [data-mode="review"]').innerText(),'Review · 0');
    ...
    for(const t of tables){
      await page.locator('#table-choice').selectOption(t.id);await page.locator('#tables-panel [data-mode="rebuild"]').click();
    ...
    for(const id of ['definite','personal','possessives','adjective-mixed']){
      await page.locator('#table-choice').selectOption(id);await page.locator('#tables-panel [data-mode="rebuild"]').click();
    ...
    await page.locator('#tables-panel [data-mode="review"]').click();
    ...
    await page.locator('#table-exercise input').fill('Ihrem');await page.keyboard.press('Enter');
    assert.equal(await page.locator('#tables-panel [data-mode="review"]').innerText(),'Review · 0');
    await page.locator('#tables-panel [data-mode="recall"]').click();
    ...
    await page.locator('#tables-panel [data-mode="study"]').click();
    ...
    const p=await blocked.newPage();await p.goto(url+'#tables');assert.match(await p.locator('#table-storage').innerText(),/temporary/);await p.locator('#tables-panel [data-mode="recall"]').click();assert.equal(await p.locator('#table-exercise input').count(),1);await blocked.close();
